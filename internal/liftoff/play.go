...
// RunPlay starts the selected services sequentially, emitting PlayUpdate
// events as each one transitions. Channel closes when finished or on first
// fatal failure.
func (l Layout) RunPlay(p PlayPlan) <-chan PlayUpdate {
	ch := make(chan PlayUpdate, 32)
	go func() {
		defer close(ch)
		// Pre-step: replace existing celery if asked.
		...
		// Start each service in canonical order.
...
// RunPause kills selected services and removes their PID files.
// Best-effort: continues past individual failures.
func (l Layout) RunPause(p PausePlan) <-chan PlayUpdate {
	ch := make(chan PlayUpdate, 16)
	go func() {
		defer close(ch)
		// Kill in reverse start order.
		services := orderedServices(p.Services)
